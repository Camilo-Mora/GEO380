...
Lets image that the plot below is the number of seedlings (points) at each height in my nursery. Interesting, ahh?. That type of distribution is called bimodal (there are two picks in the data). Clearly, there is something going on in my nursery, some seedlings are doing particularly better than others.
...
The symbol that looks like a pacman is call **summation**. The lower letter $i$ indicates that the summation starts from the first number (i.e., i=1), and the higher letter *n*, means until the last number. $Xi$ means each value. In short, the numerator of the equation above says: sum all values starting from the first one until the last one. $n$ indicates the sample size or total number of values in your database (it is was the average of a sample, you will likely see the lowercase $n$ instead of $N$).
...
In most R functions, it is important to include the command 'na.rm = TRUE', which means to exclude any missing value. If your any reason your data has empty cells or missing values, falling to include the 'na.rm = TRUE' command will result in an error....again this will be a problem only when you are missing data.
```
...
2. Multiple the number of values in your database by the given percentile you want to find as a fraction (It you want the 90th percentile, the fraction is 0.9), we call this resulting number the index or $Xth$ point
...
Because I am interested in height, measure their heights. Say they were  37, 15, 35, 36, 5, 40, 41, 68, 45, and 56cm (I am  making these numbers up, so do not overthink the size of my rabbits).
...
Next, I multiple the percentile of interest (80th) as a fraction (i.e., 0.8) by the number of rabbits (10). So the 80th percentile height is the height of whatever rabbit is sitting in 8th position. In this case the number is 45. So the 80th percentile of my rabbit population is 45cm.
...
I few things you may have notice about these two equationss.
...
In the example with the dogs, we have the problem of low sample size. In you recall and earlier chapter, the problem with low samples is that the variability increases and that is reflected in the standard deviation (SD). Lets try the standard deviation in a large sample size.
