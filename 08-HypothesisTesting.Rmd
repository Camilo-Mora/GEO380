...
In my rabbit population, the top 10% largest rabbits are larger than 7, the rabbit I found was 6.8 so in this case this rabbit belongs to my population at a 10% chance that I may be wrong.
...
The threshold beyond which we reject a null hyphothesis is called the **level of significance** or alpha, $\alpha$. You will also see named as a $critical value$.
```
...
In statistics, you can only **reject the null hypothesis** or **fail to reject the null hypothesis** (synonym for accept, but this term is not accepted because we are never certain). Along the same lines, when you reject the null hypothesis, we you do not accept the alternative hypothesis. 
...
In a nutshell, the idea of testing a hypothesis is to see if a given individual or sample is different compared to a population at a certain level of significance, $\alpha$. For the sake of simplicity, at this stage in the course, we will call the given value for the individual or sample the p-value; later on, we will transform this value for the individual or sample into an  standardized metric of how far from the mean your individual or sample is.
...
Lets say that you work for the FDA, and you have been commissioned to provide evidence for a lawsuit case involving the calories in MacDonals burgers (this is a fictitious example). The pending lawsuit refers to a person who suffered a heart attack after his cholesterol levels increased considerably. Prior to the heart attack, his doctor recommended that he kept a daily diet of no more than 2000 calories a day, so he satisfied his weakness for burgers eating a new type of MacDonals burger that claimed to have less than 1,500 calories (Standard deviation, SD= 100). 
...
Next, we take one random sample of ten values from a normal population with a mean of 1500 and an standard deviation, SD, of 50. In R, you can take any number of individuals randomly from any normally distributed population using the function $rnorm$.
...
Looking good....lets draw a line for the 5% level of confidence for the expected distribution. We will use the R function $quantile$, which we used earlier, to find out the value located at the 95% percentile, which will outline the position to the right where 5% of the cases occur.
