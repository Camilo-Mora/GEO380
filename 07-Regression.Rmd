...
Becuase extrapolation predict values beyind what is know, uncertainty is much higher and at times produce unrealistic forecasts.
```
...
## Outliers {-}
One of the critical issues with regression models, is that by looking at the residuals of the data to the linear model, it can be influenced by extreme points. Those extreme points that clearly do not follow the main pattern are called **outliers**. 
...
To compare your model to theirs, you need the F-value from your model, which we will get into in a sec, the number of variables in your model (for the purpose of comparison to the their tables, we will call this value $v1$). You also will need a parameter, we will call $v2$, which is the number of datapoints minus the number of predictors in the model plus one. $v1 and v2$ are just parameters needed to compare to random models of similar parameters.
...
If you think about that formulate above, you are estimating an standard metric of the variance explained, $r^2$, to the variance not explained, $1-r^2$, for a model of certain characteristics of sample size and variables used. 
...
So the F-value for our model was $`r round((R2/(1-R2)) *((3/1)),2)`$. Now we need to find out the *critical F-value*, and lets take a $\alpha =0.05$. For this we use an F-table, like the one below.

Because v1 = 1 in our case, you have to select the first column, then scroll down until v2 in the grey column is equal to 3, at the interception is the **F-critical**. Or the F-value at which 95% of the random models occurred. Threes are similar tables like this for each $\alpha$. 
