...
### Variace partictioning {-}
As indicated earlier, the within and between group variance and degrees of freedom should add up to the total variance and total dgrees of freedom, lets check this, by putting the results above together:
...
The F-statistics is simply the ratio of the btween group to the within group variance.
```{r, out.width = "70%", echo= FALSE, fig.align = 'center',fig.cap = 'Within group variance and degrees of freedom, DF'}
knitr::include_graphics("images/F-statistic_Example.png")
```
So, our F-statistic is 12. The fact that is larger than one provides good hints that this could be significant. But we still need to find out for the given degrees of freedoms, groups and level of significance ($\alpha$), what is the critical F-Value. If our calculated F-value is larger, we reject the null hypothesis and conclude that indeed there are significant differences...same drill as we have done before.
...
You need to select the specific table for the given ($\alpha$), the columns will represent the between degrees of freedom, and the rows the within degrees of freedom. In our case, the between degrees of freedom is 2, and our within degrees of freedoms is 6. 
...
Just to remember, our calculated F-statistics was 12 is is much larger than the critical F-statistics at $\alpha 0.05 of 5.1433. Thus, we reject the null hypothesis and conclude that indeed there are significant differences in the amount of tomatoes produced by the three varieties at level of significance of 0.05.
...
``` 
Boxplot allows you to visualize the mean and the standard deviations of each group. From the plot above, you can see clearly that not only the mean tomatoes production of the three varieties are difference, but their variances do not overlap.
...
We can also run an ANOVA in R using the aov fucntion, which is part of the default fucntions in R.
```{r, message=FALSE,warning=FALSE}
Data=data.frame(Variety=c(rep("VarA",3),rep("VarB",3),rep("VarC",3)), NumTomatoes=c(1,2,3, 3,4,5, 5,6,7))
...
You can see a different set of results from that function.
...
## Post-hoc test in R {-}
A significant one-way ANOVA means that at least one of the group means is different from the other ones. But which ones are different?. To find this out, you need to run what is called a post-hoc test, which basically compares all possible pair-wise comparison to find out, which one are different. In R, we can run a post-hoc test with the R function: tukey_hsd() [from the packgae rstatix].
...
pwc
``` 
From that comparison, you can tell that there are significant differences between Variety A and Variety C only. Cool, ah?. By looking at the plot, one could have concluded that they are all different. But if you look at the actual data, you will see that variety B, have data in common to varieties A and C, which given the small sample size, led to those pairs to be similar.


## Check assumptions {-}
Perhaps the first step in doing an ANOVA is to test the different assumptions. But I decided to leave that for last in this chapter, so we could get to understand what the ANOVA is meant to do.

### Outliers {-}
Outliers can be easily identified using box plot method, we used early but spotted mathematically with the R function identify_outliers() [rstatix package].
```{r, message=FALSE,warning=FALSE}
Data %>%  group_by(Variety) %>% identify_outliers(NumTomatoes)
``` 
The result of this function yields zero rows meaning that there we not outliers in our database. Should there be outlier, it is recomended to run the analysis with and without them, to see the level of the effect and for you to make judgement call of whether to remove them or now,a nd justify any choice you make.
...
From the plot, you can see all values are within the confidence limits expected under normality.
